  ...
  const checkIfAlreadyApplied = async () => {
    if (id && session) {
      try {
        ...
        const appliedRole = data.appliedTalentsId.find(entry => entry.userId === session.user.id);
        if (appliedRole) {
  ...
  useEffect(() => {
    const fetchCasting = async () => {
      try {
        ...
        const response = await fetch(`/api/castings/${id}`, { cache: "no-store" });
        if (!response.ok) {
        ...
        <div className="talent-work">
          ...
          <h3>Casting Title: {casting.castingTitle}</h3>
            <div className="ml-5 flex">
              <h3>Role Types:</h3>
              <ul>
                {casting.roleTypes && casting.roleTypes.map((role, index) => (
                  <li key={index}>{role}</li>
                ))}
              </ul>
            </div>
            <h3>shootDate: <span>{formatDate(casting.shootDate)}</span></h3>
            <h3>lastDate: <span>{formatDate(casting.lastDate)}</span></h3>
            <h3>castingCreaterId: <span>{casting.castingCreaterId}</span></h3>
          </div>
          {/* Role */}
