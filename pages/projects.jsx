      ...
      {filteredData.slice(0, visibleCount).map((item, index) => {
        ...
        return (
          <div key={item._id} className="flex flex-col m-5" ref={isLastElement ? lastElementRef : null}>
              ...
              <h2 className="h2">Project Id: {item._id}</h2>
              <h2 className="h2">Project Image</h2>
              <Image
               width={300}
               height={50}
               alt='Project Image'
                src= {item.projectImage}/>
                <h2 className="h2">Project Name: {item.projectName}</h2>             
                <h3 className="h2">Description: {item.description}</h3>
                <h3 className="h2">Category: {item.category}</h3>     

                <div className="talent-button">
                  <button className="view-profile">
                    <Link href={`projectsingle/${item._id}`} target="_blank">
                      View Profile
                    </Link>
                  </button>
                </div>
